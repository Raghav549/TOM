...
class TomLiveBridgeController(
    ...
    private fun handleAction(envelope: TomLiveEnvelope) {
        ...
        val observationPayload = JSONObject()
            ...
            .put("action_id", request.actionId)
            .put("reason", "post_action_verification")
        sendObservation(envelope.copy(type = "OBSERVATION_REQUEST", correlationId = request.actionId, payload = observationPayload))
    }

    private fun sendObservation(request: TomLiveEnvelope) {
        val observationId = request.correlationId ?: UUID.randomUUID().toString()
        val taskId = request.payload.optString("task_id").takeIf { it.isNotBlank() }
        val actionId = request.payload.optString("action_id").takeIf { it.isNotBlank() }
