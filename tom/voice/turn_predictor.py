...
class LearnedTurnPredictor:
    ...
    def predict(self, *, vad: float, vad_delta: float, energy: float, energy_delta: float,
                pitch_variation: float, speech_rate: float, asr_confidence: float,
                tom_speaking: bool) -> TurnPrediction:
        self._load()
        import numpy as np

        features = np.asarray([[vad, vad_delta, energy, energy_delta, pitch_variation,
                                speech_rate, asr_confidence, 1.0 if tom_speaking else 0.0]], dtype=np.float32)
