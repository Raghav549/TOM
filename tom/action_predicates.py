...
class OpenAppPredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        ok = actual == expected
        return PredicateResult(ok, 1.0 if ok else 0.0, "expected Android package is foreground" if ok else f"foreground package {actual!r} != {expected!r}", (PredicateEvidence("accessibility", f"package={actual}"),))
...
class TextEntryPredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        return PredicateResult(matched, 0.95 if matched else 0.0, "entered text observed" if matched else "entered text not observed", (PredicateEvidence("ui_text", f"expected_text_observed={matched}"),))
...
class NavigationPredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        return PredicateResult(changed, 0.8 if changed else 0.05, "navigation state changed" if changed else "navigation state not proven", (PredicateEvidence("screen_state", f"fingerprint_changed={before_fp != after_fp}"), PredicateEvidence("app_url", f"package_changed={package_changed};url_changed={url_changed}")))
...
class ProviderPaymentPredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        provider = str(after.get("payment_provider") or after.get("provider") or tool.get("payment_provider") or tool.get("provider") or "").strip()
        transaction = str(after.get("transaction_id") or after.get("upi_txn_id") or tool.get("transaction_id") or tool.get("upi_txn_id") or "").strip()
...
class MessageSendPredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        evidence = (PredicateEvidence("ui_text", f"message_seen={message_seen};recipient_seen={recipient_seen}"), PredicateEvidence("delivery", f"delivered={delivered}"))
...
class CalendarCreatePredicate(SuccessPredicate):
    ...
    def evaluate(self, context: VerificationContext) -> PredicateResult:
        ...
        return PredicateResult(ok, 0.95 if ok else 0.0, "calendar event creation evidence observed" if ok else "calendar event creation not proven", (PredicateEvidence("ui_text", f"title_seen={matched};saved={saved}"),))
...
class PredicateRegistry:
    ...
    def get(self, action: str) -> SuccessPredicate | None:
        ...
        aliases = {
            "open_app": "open_app", "tap": "tap", "tap_node": "tap", "search_google": "search", "search": "search", "device_search_google": "search",
            "set_text_node": "set_text", "device_type": "set_text", "open_url": "navigation", "open_intent_uri": "navigation", "back": "navigation", "home": "navigation", "recents": "navigation",
            "send_message": "send_message", "device_send_message": "send_message", "send_email": "send_message", "create_calendar_event": "create_calendar_event", "device_create_calendar_event": "create_calendar_event",
            "payment": "upi_payment", "device_upi_payment": "upi_payment", "upi_payment": "upi_payment",
        }
...
def default_predicates() -> PredicateRegistry:
    ...
    for predicate in (OpenAppPredicate(), TapPredicate(), SearchPredicate(), TextEntryPredicate(), NavigationPredicate(), ProviderPaymentPredicate(), MessageSendPredicate(), CalendarCreatePredicate()):
...
def _truthy(value: Any) -> bool:
    return value is True or str(value).lower() in {"1", "true", "yes", "y", "on"}
