...
class BrowserRuntimeTool:
    ...
    async def run(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.name == "browser_click_text":
            ...
            await page.get_by_text(text, exact=bool(arguments.get("exact", False))).first.click()
            return (await self.runtime.snapshot()).__dict__
...
async def register_browser_tools(registry: ToolRegistry) -> PlaywrightBrowser:
    ...
    for name, description, risk in tools:
