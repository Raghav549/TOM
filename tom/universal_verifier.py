...
def verify_universal(
    ...
    ``None`` means the generic success-predicate engine remains authoritative.
    A transport/tool result is never treated as semantic success for the hard
    actions below.
    """
    ...
    if kind in {"send_message", "send_email", "send_sms", "communication.sms_send", "google.gmail_send"}:
        ...
        state = _norm(
            observation.get(
                "message_state",
                observation.get("send_state", observation.get("provider_status")),
            )
        )
        ...
        if state in {"sent", "delivered", "submitted", "success", "completed"} and (actual_recipient or recipient) and (actual_body or body):
            return True, 0.995, "recipient, body and sent state confirmed", (
                "recipient",
                "message_body",
                "message_state",
            )
    ...
    if kind in {"video_call", "device_video_call"}:
        ...
        if call_state in {"connected", "active", "in_call", "ongoing"} and video and audio:
            return True, 0.995, "connected call with active video and audio confirmed", (
                "call_state",
                "video_active",
                "audio_active",
            )
    ...
    if kind in {"book", "booking", "device_book"}:
        confirmation = str(
            observation.get("booking_confirmation", observation.get("confirmation_id", ""))
        ).strip()
        ...
        if confirmation and state in {"confirmed", "booked", "success", "completed"}:
            return True, 0.999, "provider booking confirmation evidence", (
                "confirmation_id",
                "booking_state",
            )
    ...
    if kind in {"create_calendar_event", "device_create_calendar_event"}:
        event_id = str(
            observation.get(
                "event_id",
                observation.get("provider_event_id", observation.get("id", "")),
            )
        ).strip()
    ...
    if kind in {"delete", "device_delete"}:
        ...
        if _norm(observation.get("operation_state")) in {"deleted", "success", "completed"} and observation.get("deleted_target"):
            return True, 0.98, "deletion state and target evidence confirmed", (
                "operation_state",
                "deleted_target",
            )
    ...
    if kind in {"publish", "device_publish"}:
        ...
        if state in {"published", "live", "success", "completed"} and (
            observation.get("published_id")
            or observation.get("published_url")
            or "published" in _blob(observation)
        ):
    ...
    if kind in {"upi", "device_upi_payment", "payment", "device_payment"}:
        ...
        state = _norm(
            observation.get(
                ...
                observation.get("payment_state", observation.get("provider_status")),
            )
        )
        txn = str(
        ...
        if state in {"success", "succeeded", "completed", "paid"} and txn:
            return True, 0.999, "provider, amount, recipient and transaction success evidence confirmed", (
                "payment_provider",
                "payment_amount",
                "payment_recipient",
                "transaction_id",
