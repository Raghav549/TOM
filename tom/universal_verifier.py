...
def verify_universal(action: str, expected: Mapping[str, Any], observation: Mapping[str, Any]) -> tuple[bool, float, str, tuple[str, ...]] | None:
    """Return a hard predicate for actions that need more than generic UI change.

    None means the existing SuccessPredicateEngine remains authoritative.
    """
    ...
    if kind in {"upi", "device_upi_payment", "payment", "device_payment"}:
        ...
        state = _norm(observation.get("provider_payment_state", observation.get("payment_state")))
        txn = str(observation.get("transaction_id", observation.get("utr", observation.get("txn_id", "")))).strip()
        if expected.get("provider") and provider != _norm(expected["provider"]):
