...
class AndroidBridgeHub:
    """Authenticated Android WSS hub with action/verification correlation."""
            ...
            if result.get("accepted") and result.get("status") in {"completed", "verified"} and self.core_receiver:
                ...
                return {
                    **result,
                    ...
                    "verified": verified["verification"]["status"] == "verified",
    ...
    async def request_verification(self, device_id: str, task_id: str, action_id: str, timeout: float = 15.0) -> dict[str, Any] | None:
        ...
        await self.send(device_id, {
            ...
            "sequence": 0,
            "device_id": device_id,
            "payload": {"task_id": task_id, "action_id": action_id, "reason": "post_action_verification"},
        })
        await self.send(device_id, {
    ...
    async def resolve_incoming(self, message: dict[str, Any]) -> None:
        ...
        if message_type == "action_result":
            ...
            await self.emit(f"device.{message_type or 'event'}", payload, task_id=str(payload.get("task_id") or "") or None)
            key = ""
    ...
    async def resolve_verification(self, result: dict[str, Any]) -> None:
        ...
        await self.emit(f"verification.{status}", result, task_id=task_id)
        key = f"verification:{task_id}:{action_id}"
        async with self.lock:
            future = self._waiters.get(key)
        if future and not future.done():
...
async def _forward_to_core(hub: AndroidBridgeHub, message: dict[str, Any]) -> None:
    ...
    except Exception as exc:  # noqa: BLE001 - perception is an external model boundary
        task_id = str((message.get("payload") or {}).get("task_id") or "") or None
        await hub.emit("verification.error", {"error": str(exc)}, task_id=task_id)


hub = AndroidBridgeHub()


def install_android_bridge(
    app: Any,
    *,
    event_stream: LiveEventStream | None = None,
    core_receiver: CoreBridgeReceiver | None = None,
) -> AndroidBridgeHub:
    ...
    async def handler(websocket: WebSocket) -> None:
        ...
        try:
            ...
            await websocket.send_text(json.dumps({
                ...
                "device_id": device_id,
                "sequence": 0,
                "payload": {"accepted": True, "server_capabilities": ["live_observation", "action_request", "verification", "multimodal_verification", "request_correlation", "core_event_stream"]},
            }, separators=(",", ":")))
            ...
            while True:
                ...
                if event_stream:
                    await event_stream.publish(
                        "android." + str(message.get("type") or "event"),
