...
class LiveTaskBridge:
    ...
    def bind(self, task_id: str, goal: str, device_id: str) -> BoundLiveTask:
        current = self.tasks.get(task_id)
        if current:
            return current
        context = LiveExecutionContext(task_id, goal)
        loop = LiveDeviceLoop(self.planner, self.event_bus, context)
        bound = BoundLiveTask(task_id, goal, device_id, loop)
    ...
    async def on_verification(self, result: dict[str, Any], tools: list[dict[str, Any]]) -> Any:
        ...
        if status != "failed":
            return None
        return await task.loop.replan(goal=task.goal, memory=[], tools=tools)
