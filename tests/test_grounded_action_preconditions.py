...
def call(name: str, arguments: dict, risk: Risk = Risk.MEDIUM) -> ToolCall:
...
def test_device_tap_requires_grounded_success_predicate() -> None:
    ...
    result = checker.check(call("device_tap_node", {"node_id": "3"}))
...
def test_device_tap_accepts_grounded_predicate() -> None:
    checker = ActionPreconditionChecker()
    result = checker.check(call("device_tap_node", {
        "node_id": "3",
...
def test_open_app_requires_predicate_even_when_package_is_known() -> None:
    checker = ActionPreconditionChecker()
    result = checker.check(call("device_open_app", {"package_name": "com.whatsapp"}))
    assert not result.ok
...
def test_payment_requires_approval_and_predicate() -> None:
    checker = ActionPreconditionChecker()
    result = checker.check(call("device_upi_payment", {
